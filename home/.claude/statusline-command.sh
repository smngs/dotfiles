...
format_reset_time() {
  ...
  local result=""
  # Try ISO 8601 with Z suffix (e.g. 2026-03-07T15:00:00Z)
  result=$(LC_ALL=C TZ="Asia/Tokyo" date -j -f "%Y-%m-%dT%H:%M:%SZ" "$ts" "$fmt" 2>/dev/null)
  # Try ISO 8601 with numeric offset (strip colon for BSD date: +09:00 → +0900)
  if [ -z "$result" ]; then
    local ts_fixed
    ts_fixed=$(echo "$ts" | sed 's/\([+-][0-9][0-9]\):\([0-9][0-9]\)$/\1\2/')
    result=$(LC_ALL=C TZ="Asia/Tokyo" date -j -f "%Y-%m-%dT%H:%M:%S%z" "$ts_fixed" "$fmt" 2>/dev/null)
  fi
  # Fallback: treat as epoch seconds
  if [ -z "$result" ]; then
    result=$(LC_ALL=C TZ="Asia/Tokyo" date -j -f "%s" "$ts" "$fmt" 2>/dev/null || \
             LC_ALL=C TZ="Asia/Tokyo" date -d "@${ts}" "$fmt" 2>/dev/null || echo "")
  fi
